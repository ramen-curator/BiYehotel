...
exports.author_list = (req, res) => { 
    ...
    .exec(function (err, list_authors) {
      ...
      res.render('author_list', { title: '用户列表', author_list: list_authors });
    });
};
...
exports.author_detail = (req, res) => { 
    async.parallel({
        ...
        res.render('author_detail', { title: '用户详情', author: results.author, article_list: results.articles } );
...
exports.author_create_get = (req, res) => { 
    res.render('author_form', { title: '创建用户'});
};
...
exports.author_create_post = [
    ...
    (req, res, next) => {
      ...
      if (!errors.isEmpty()) {
        res.render('author_form', { title: '创建用户', author: req.body.author, errors: errors.array() });
        return;
      }
      else {
        let identity_h;
...
exports.author_delete_get = (req, res, next) => {
  async.parallel({
    ...
    res.render('author_delete', { title: '删除用户', author: results.author, author_articles: results.author_articles } );
  });
};
...
exports.author_delete_post = (req, res, next) => {
  async.parallel({
    ...
    if (results.author_articles.length > 0 
      ...
      && c == true)) {
      res.render('author_delete', { title: '删除用户', author: results.author, author_articles: results.author_articles, input_err: "口令错误" } );
      return;
    }
    else {
...
exports.author_update_post = (req, res,next) => {
  if(req.body.name==undefined){
    async.parallel(
      {
        ...
        if(req.body.v_password != results.author.password
          ...
          res.render('what_form',{title:'更改用户信息', fine: true, author: results.author});
        }
      }
      );
    }else{//已经确认身份，正式开始更改
    ...
    if (!errors.isEmpty()) {
      res.render('what_form', { title: '更改用户信息', fine:true,author: req.body, errors: errors.array() });
      return;
    }
    else {
      let identity_h;
